...
Ok, good work, going to look at it now, but please, make sure to capitalize the file names properly another time. We should rename all files so that the first letter in every word is capitalized, the code also needs to reflect that.
...
so which file(s) do we run to have your program work? -blotz
